...
public class CharacService {
    ...
    public void clearAllQuests(List<Charac> characters) {
        ...
        for (int i = 0; i < clearQuests.length; i++) {
            clearQuestBytes[i] = new BigInteger(clearQuests[i], 16).toByteArray();
        }
        byte[] questNotifyBytes = new BigInteger(questNotify, 16).toByteArray();
        byte[] specificSectionBytes = new BigInteger(specificSection, 16).toByteArray();
        byte[] generalSectionBytes = new BigInteger(generalSection, 16).toByteArray();
        byte[] despairBytes = new BigInteger(despair, 16).toByteArray();
